...
wrongGuesses = 0
MAX_GUESSES = 13  #Add a max number of guesses
...
topScreenTurtle.goto(-1 * int(sWidth / 2) + int(sWidth * 0.1),  int(sHeight * 0.35))
...
def drawWrongLetters():
    ...
    #topScreenTurtle.write(wrongLetters, move=False, align="left", font=("Arial", topFont, "normal"))
    lString = "Not In Word: "
    for l in wrongLetters:
        lString += l + ","
    #slice off the last comma
    lString = lString[ : len(lString)-1]
...
while gameOn:
    ...
    if guess == "$$":
        ...
        writeErrorMessage("I need a letter, not" + guess + " Guess Again.")
    else:
        if guess.strip().lower() in secretWord.lower():
            correctLetters.append(guess.lower())
            drawWord()
        else:
            wrongLetters.append(guess)
            wrongGuesses += 1
            drawWrongLetters()  #show the letters
            updateDrawing()     #draw the hangman/person
            time.sleep(1)
            if(wrongGuesses >= MAX_GUESSES):
                gameOn = False
                writeErrorMessage("Sorry you are out of guesses.  Game over.")
        if("_" not in screenWord):
            writeErrorMessage("Game over.  YOU WIN!!!")
            gameOn = False
